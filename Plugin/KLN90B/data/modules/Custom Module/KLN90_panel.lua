  ...
  while true do
    ...
    if line == '\r' or line == '' then
      while true do
        ...
        words[1], words[2] = line:match("(%w+)(.+)")
        if words[1] == nil or words[2] == nil then
          print("APT Parsing ERROR in " .. filename .. "\r" .. "LINE: " .. line)
          goto continue
        end
        if words[1] == "1" or words[1] == "16" or words[1] == "17" then
          words[3], words[4], words[5], words[6], words[7] = words[2]:match("(%w+) (%w+) (%w+) (%w+) (.+)")
          for i = 3, 7, 1 do if words[i] == nil then
              print("APT Parsing ERROR in " ..
              filename .. "\r" .. "CASE " .. words[1] .. " FIELD: " .. i - 1 .. " LINE: " .. line)
              goto continue
            end end                                                                                                                                                                         --check if correct format was read
          if string.len(words[6]) > 4 then
            print("APT Parsing: Skipping: " .. words[6] .. " - ICAO code longer than 4 digits")
            goto continue
          end
        ...
        elseif words[1] == "100" then
          ...
          for i = 1, 19, 1 do if fields[i] == nil then
              print("APT Parsing ERROR in " ..
              filename .. "\r" .. "CASE " .. words[1] .. " FIELD: " .. i + 1 .. " LINE: " .. line)
              goto continue
            end end                                                                                                                                                                           --check if correct format was read
        ...
        elseif words[1] == "102" then
          ...
          for w in words[2]:gmatch("([^%s]+)") do fields[#fields + 1] = w end
          for i = 1, 11, 1 do if fields[i] == nil then
              print("APT Parsing ERROR in " ..
              filename .. "\r" .. "CASE " .. words[1] .. " FIELD: " .. i + 1 .. " LINE: " .. line)
              goto continue
            end end                                                                                                                                                                           --check if correct format was read
        ...
        elseif words[1] == "101" then
          ...
          for w in words[2]:gmatch("([^%s]+)") do fields[#fields + 1] = w end
          for i = 1, 8, 1 do if fields[i] == nil then
              print("APT Parsing ERROR in " ..
              filename .. "\r" .. "CASE " .. words[1] .. " FIELD: " .. i - 1 .. " LINE: " .. line)
              goto continue
            end end                                                                                                                                                                          --check if correct format was read
  ...
  while true do
    ...
    for i = 1, 5, 1 do --check if correct format was read
      if fields[i] == nil then
        print("FIX Parsing ERROR in " .. filename .. "\r" .. " FIELD: " .. i .. " LINE: " .. line)
        goto continue
      end
    end
...
function parse_nav(path, data)
  ...
  while true do
    ...
    words[1], words[2] = line:match("(%w+)(.+)")
    if words[1] == nil or words[2] == nil then
      print("NAV Parsing ERROR in " .. filename .. "\r" .. "LINE: " .. line)
      goto continue
    end
    if words[1] == "2" then
      local fields = {}
      for w in words[2]:gmatch("([^%s]+)") do fields[#fields + 1] = w end
      for i = 1, 10, 1 do if fields[i] == nil then
          print("NAV Parsing ERROR in " ..
          filename .. "\r" .. "CASE " .. words[1] .. " FIELD: " .. i .. " LINE: " .. line)
          goto continue
        end end                                                                                                                                                                          --check format
      fields[1] = string.format("%.6f", fields[1])
      fields[2] = string.format("%.6f", fields[2])
      if #fields > 10 then
...
function create_aptdb(path, data)
  ...
  --icao 1, name 2, amsl 3, lat 4, lon 5, rwy{} 6, atis{} 7, type 8
  file:write("X|" ..
  fields[6]:sub(1, 4) .. "|" .. "XXXXX01" .. numbertomonth(tonumber(month)) .. "/" .. year .. "|XXXXXXXXXXXXXXXXXX\n\n")
  for k, v in pairsByKeys(data["airport"]) do
    ...
    for k1, v1 in pairs(v[6]) do
      ...
      elseif surf == "WAT" then
        ...
        v1[5] ..
        "|" ..
        v1[6] .. "|" .. v1[7] .. "|" .. v1[8] .. "|" .. v1[9] .. "|" .. v1[3] .. "|" .. surf .. "|" .. light .. "\n")
      else
        file:write("R|" ..
        v1[4] ..
        "|" ..
...
function course(lat1, lon1, lat2, lon2)
  ...
  lat2 = lat2 * pi / 180
  lon2 = lon2 * pi / -180
  local course = math.fmod(
  math.atan2(sin(lon1 - lon2) * cos(lat2), cos(lat1) * sin(lat2) - sin(lat1) * cos(lat2) * cos(lon1 - lon2)), 2 * pi)
...
function raddist(lat1, lon1, tc, d)
  ...
  dlon = math.atan2(sin(tc * pi / 180) * sin(d * pi / 10800) * cos(lat1 * pi / 180),
    cos(d * pi / 10800) - sin(lat1 * pi / 180) * sin(lat))
    ...
    while num <= FPlan[0]["length"] do
      if FPlan[0][num]["types"] == 4 or FPlan[0][num]["types"] == 5 or FPlan[0][num]["USR"] == 1 then
        ...
        while num2 <= 6 do
          ...
          if str == "." then
            str = "period"                  --workaround error sim/FMS/key_. does not exsist
          elseif str == "" then
            break
...
function turnanti(bearing)
  ...
  local CRS2 = course(values["activeWPT"][2]["lat"], values["activeWPT"][2]["lon"], values["activeWPT"][3]["lat"],
    values["activeWPT"][3]["lon"])
        ...
        if #folder1 > 0 then
          for j = 1, #folder1 do
            if folder1[j].name == "Earth nav data" then
              ...
              if #files2 > 0 then
                for y = 1, #files2 do
                  if files2[y].name == "apt.dat" and files[i].name ~= "Global Airports" then folders["apt"][#folders["apt"] + 1] =
                    path .. files[i].name .. "/" .. folder1[j].name .. "/" end
                  if files2[y].name == "earth_nav.dat" then folders["nav"][#folders["nav"] + 1] = path ..
                    files[i].name .. "/" .. folder1[j].name .. "/" end
                end
              end
            end
          end
  ...
  if #files > 0 then
    for i = 1, #files do
      if files[i].type == "file" then
        if files[i].name == "airports.txt" or files[i].name == "navaids.txt" or files[i].name == "waypoints.txt" then found =
          found + 1 end
      end
    end
  else
...
function createSIDSTAR(name)
  ...
  if name == "SID" then
    while num <= values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"][values["SIDSTARsel"][name .. "RWY"]]["num"] do
      local waypoint = values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"][values["SIDSTARsel"][name .. "RWY"]]
      [num]
  ...
  if values["SIDSTARsel"][name .. "TRANS"] ~= 0 then
    ...
    while num <= values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"][values["SIDSTARsel"][name .. "TRANS"]]["num"] do
      local waypoint = values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"]
      [values["SIDSTARsel"][name .. "TRANS"]][num]
      local flyo = waypoint["flyo"]
      waypoint = enterident(makelength(waypoint["ident"], 5, 0), 9, 0, 5, 0, waypoint["lat"], waypoint["lon"])
      if waypoint["length"] > 0 then
        values["SIDSTARsel"]["num"] = values["SIDSTARsel"]["num"] + 1
  ...
  if name == "STAR" or name == "APP" then
    ...
    while num <= values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"][values["SIDSTARsel"][name .. "RWY"]]["num"] do
      ...
      if name == "APP" and string.sub(waypoint[1]["ident"], 1, 2) == "RW" then
        ...
        waypoint[1]["lat"] = values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]
        [values["SIDSTARsel"][name .. "RWY"]][num]["lat"]
        waypoint[1]["lon"] = values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]
        [values["SIDSTARsel"][name .. "RWY"]][num]["lon"]
        values["SIDSTARsel"]["num"] = values["SIDSTARsel"]["num"] + 1
        values["SIDSTARsel"][values["SIDSTARsel"]["num"]] = table.copy(waypoint[1])
        values["SIDSTARsel"][values["SIDSTARsel"]["num"]]["flyo"] = flyo
    ...
    while f1 <= 1 do
      ...
      if lat then
        dist = distance(lat, lon,
          intermediat(FPlan[num2][num - 1]["lat"], FPlan[num2][num - 1]["lon"], FPlan[num2][num]["lat"],
            FPlan[num2][num]["lon"], f1))
      else
        dist = distance(values["GPSlat"], values["GPSlon"],
          intermediat(FPlan[0][num - 1]["lat"], FPlan[0][num - 1]["lon"], FPlan[0][num]["lat"], FPlan[0][num]["lon"], f1))
      end
    ...
    if WPTtable[num]["USR"] == 1 then
      if WPTtable[num]["lat"] ~= "_" and WPTtable[num]["lon"] ~= "_" then
        if WPTtable[num]["types"] == 0 then
          file:write(string.format("A|%s|%s|%s|%s|%s|%s\n", WPTtable[num]["ident"], WPTtable[num]["lat"],
            WPTtable[num]["lon"], WPTtable[num]["elev"], WPTtable[num]["LRWY"], WPTtable[num]["surface"]))
        elseif WPTtable[num]["types"] == 1 then
          file:write(string.format("V|%s|%s|%s|%s|%s\n", WPTtable[num]["ident"], WPTtable[num]["lat"],
            WPTtable[num]["lon"], WPTtable[num]["freq"], WPTtable[num]["magvar"]))
...
function drawmap(size, orientation, range, mode)
  ...
  if # Nav5Comp == 0 and values["GPSnum"] >= 4 then
    ...
    if mode == 1 then
        ...
        while num <= 9 do
          ...
          if show == 1 then
            if values["VORnearestlist"][num]["range"] > VORrange then
              local dist1 = distance(values["GPSlat"], values["GPSlon"], values["VORnearestlist"][num]["lat"],
                values["VORnearestlist"][num]["lon"]) * PXnm
              if dist1 < 90 then
                local CRS1 = course(values["GPSlat"], values["GPSlon"], values["VORnearestlist"][num]["lat"],
                  values["VORnearestlist"][num]["lon"]) - orient
      ...
      if string.sub(values["NAV5SHOW"], 2, 2) ~= "0" and values["NAV5Clut"] == 0 then
        ...
        while num <= 9 do
          ...
          if show == 1 then
            local dist1 = distance(values["GPSlat"], values["GPSlon"], values["NDBnearestlist"][num]["lat"],
              values["NDBnearestlist"][num]["lon"]) * PXnm
            if dist1 < 90 then
              local CRS1 = course(values["GPSlat"], values["GPSlon"], values["NDBnearestlist"][num]["lat"],
                values["NDBnearestlist"][num]["lon"]) - orient
              local y1 = cy + cos((CRS1) * pi / 180) * dist1
              local x1 = cx + cos((CRS1 - 90) * pi / 180) * dist1
      ...
      if string.sub(values["NAV5SHOW"], 3, 3) ~= "0" and values["NAV5Clut"] == 0 then
        ...
        while num <= 9 do
          ...
          if show == 1 then
            local dist1 = distance(values["GPSlat"], values["GPSlon"], values["APTnearestlist"][num]["lat"],
              values["APTnearestlist"][num]["lon"]) * PXnm
            if dist1 < 90 then
              local CRS1 = course(values["GPSlat"], values["GPSlon"], values["APTnearestlist"][num]["lat"],
                values["APTnearestlist"][num]["lon"]) - orient
    ...
    if values["activeWPT"]["active"] == 0 and values["activeWPT"]["length"] == 2 then
      local dist1 = distance(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"],
        values["activeWPT"][2]["lon"]) * PXnm
      local CRS1 = course(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"],
        values["activeWPT"][2]["lon"]) - orient

      local y1 = cy + cos((CRS1) * pi / 180) * dist1
      local x1 = cx + cos((CRS1 - 90) * pi / 180) * dist1
      dist1 = distance(values["GPSlat"], values["GPSlon"], values["activeWPT"][1]["lat"], values["activeWPT"][1]["lon"]) *
      PXnm
      CRS1 = course(values["GPSlat"], values["GPSlon"], values["activeWPT"][1]["lat"], values["activeWPT"][1]["lon"]) -
      orient
        ...
        if range <= 2 and values["activeWPT"][2]["types"] == 0 then
          ...
          while RWYnum <= values["activeWPT"][2]["RWYs"] do
            ...
            if RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["lat2"] then
              local dist1 = distance(values["GPSlat"], values["GPSlon"],
                RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["lat"],
                RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["lon"]) * PXnm
              local CRS1 = course(values["GPSlat"], values["GPSlon"],
                RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["lat"],
                RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["lon"]) - orient
              local y1 = cy + cos((CRS1) * pi / 180) * dist1
              local x1 = cx + cos((CRS1 - 90) * pi / 180) * dist1
              dist1 = distance(values["GPSlat"], values["GPSlon"],
                RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["lat2"],
                RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["lon2"]) * PXnm
              CRS1 = course(values["GPSlat"], values["GPSlon"], RWYtable[values["activeWPT"][2]["ident"]][RWYnum]
              ["lat2"], RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["lon2"]) - orient
              ...
              if range == 1 or RWYnum == 1 then
                if x1 - 0.5 > size[1] and x1 + 0.5 < size[1] + size[3] and y1 - 0.5 > size[2] and y1 + 0.5 < size[2] + size[4] then
                  table.insert(nametable,
                    { makelength(RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["number1"], 5, 0), x1, y1, size })
                end
                if x2 - 0.5 > size[1] and x2 + 0.5 < size[1] + size[3] and y2 - 0.5 > size[2] and y2 + 0.5 < size[2] + size[4] then
                  table.insert(nametable,
                    { makelength(RWYtable[values["activeWPT"][2]["ident"]][RWYnum]["number2"], 5, 0), x2, y2, size })
                end
              end
            end
            RWYnum = RWYnum + 1
    ...
    while WPTnum <= FPlan[0]["length"] do
      if FPlan[0][WPTnum]["lat"] ~= nil and FPlan[0][WPTnum]["lon"] ~= nil then
        local dist1 = distance(values["GPSlat"], values["GPSlon"], FPlan[0][WPTnum]["lat"], FPlan[0][WPTnum]["lon"]) *
        PXnm
        local CRS1 = course(values["GPSlat"], values["GPSlon"], FPlan[0][WPTnum]["lat"], FPlan[0][WPTnum]["lon"]) -
        orient
        ...
        if values["activeWPT"]["active"] > 0 then
          ...
          if WPTnum == values["activeWPT"]["active"] then
            if values["activeWPT"][1]["ident"] == "    $" or values["activeWPT"][1]["ident"] == "     " then
              local dist1 = distance(values["GPSlat"], values["GPSlon"], values["activeWPT"][1]["lat"],
                values["activeWPT"][1]["lon"]) * PXnm
              local CRS1 = course(values["GPSlat"], values["GPSlon"], values["activeWPT"][1]["lat"],
                values["activeWPT"][1]["lon"]) - orient
          ...
          if range <= 2 and FPlan[0][WPTnum]["types"] == 0 then
            ...
            while RWYnum <= FPlan[0][WPTnum]["RWYs"] do
              ...
              if RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["lat2"] then
                local dist1 = distance(values["GPSlat"], values["GPSlon"],
                  RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["lat"], RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["lon"]) *
                PXnm
                local CRS1 = course(values["GPSlat"], values["GPSlon"],
                  RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["lat"], RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["lon"]) -
                orient
                local y1 = cy + cos((CRS1) * pi / 180) * dist1
                local x1 = cx + cos((CRS1 - 90) * pi / 180) * dist1
                dist1 = distance(values["GPSlat"], values["GPSlon"], RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["lat2"],
                  RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["lon2"]) * PXnm
                CRS1 = course(values["GPSlat"], values["GPSlon"], RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["lat2"],
                  RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["lon2"]) - orient
                ...
                if range == 1 or RWYnum == 1 then
                  ...
                  if x1 - 0.5 > size[1] and x1 + 0.5 < size[1] + size[3] and y1 - 0.5 > size[2] and y1 + 0.5 < size[2] + size[4] then
                    table.insert(nametable,
                      { makelength(RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["number1"], 5, 0), x1, y1, size })
                  end
                  if x2 - 0.5 > size[1] and x2 + 0.5 < size[1] + size[3] and y2 - 0.5 > size[2] and y2 + 0.5 < size[2] + size[4] then
                    table.insert(nametable,
                      { makelength(RWYtable[FPlan[0][WPTnum]["ident"]][RWYnum]["number2"], 5, 0), x2, y2, size })
                  end
                end
                drawline(Nav5Comp, x1, y1, x2, y2, size)
              end
...
function editvalue(mode, side, value)
  ...
  if values[side .. "editstate"] == 0 then
    if controls[side .. "knobl"] == -1 then
      ...
      elseif controls[side .. "CRSRchar"] ~= 0 then
        ...
        if controls[side .. "CRSRchar"] < 1 then
          ...
          elseif mode == 2 then
            controls[side .. "CRSRchar"] = 4
          elseif mode == 3 then
            controls[side .. "CRSRchar"] = 3
          elseif mode == 4 then
            controls[side .. "CRSRchar"] = string.len(value)
          elseif mode == 5 then
            controls[side .. "CRSRchar"] = 7
          elseif mode == 6 then
            controls[side .. "CRSRchar"] = 7
          elseif mode == 7 then
            controls[side .. "CRSRchar"] = string.len(value)
    ...
    elseif controls[side .. "knobl"] == 1 then
      ...
      elseif controls[side .. "CRSRchar"] ~= 0 then
        ...
        if mode == 11 and controls[side .. "CRSRchar"] > 1 and controls[side .. "CRSRchar"] < 5 then
          controls[side .. "CRSRchar"] = 4
        elseif mode == 11 and controls[side .. "CRSRchar"] > 5 then
    ...
    elseif controls[side .. "knobs"] == -1 then
      if controls[side .. "CRSRchar"] == 0 then
        ...
        elseif mode == 3 then
          ...
          if values[side .. "editvalue"]["hour"] > 23 then values[side .. "editvalue"]["hour"] = values
            [side .. "editvalue"]["hour"] - 24 end
      ...
      elseif mode == 1 then
        values[side .. "editvalue"] = enterident(values[side .. "editvalue"][1]["ident"], 9, 0,
          controls[side .. "CRSRchar"], -1)
      elseif mode == 2 then
        if controls[side .. "CRSRchar"] == 1 then
      ...
      elseif mode == 4 then
        x = string2value(string.sub(values[side .. "editvalue"], controls[side .. "CRSRchar"],
          controls[side .. "CRSRchar"])) - 1
        if x < 0 then x = 36 end
        values[side .. "editvalue"] = replaceChar(values[side .. "editvalue"], controls[side .. "CRSRchar"],
          value2string(x))
      elseif mode == 5 then
        if controls[side .. "CRSRchar"] == 1 then
          if string.sub(values[side .. "editvalue"], 1, 1) == "S" then
            values[side .. "editvalue"] = replaceChar(values[side .. "editvalue"], 1, "N")
          else
            values[side .. "editvalue"] = replaceChar(values[side .. "editvalue"], 1, "S")
          end
        elseif controls[side .. "CRSRchar"] == 2 then
      ...
      elseif mode == 6 then
        if controls[side .. "CRSRchar"] == 1 then
          if string.sub(values[side .. "editvalue"], 1, 1) == "E" then
            values[side .. "editvalue"] = replaceChar(values[side .. "editvalue"], 1, "W")
          else
            values[side .. "editvalue"] = replaceChar(values[side .. "editvalue"], 1, "E")
          end
        elseif controls[side .. "CRSRchar"] == 2 then
      ...
      elseif mode == 11 then
        if controls[side .. "CRSRchar"] == 1 then
          ...
          x = string2value(string.sub(values[side .. "editvalue"], controls[side .. "CRSRchar"] - 1,
    ...
    elseif controls[side .. "knobs"] == 1 then
      ...
      elseif mode == 8 then
        x = string2value(string.sub(values[side .. "editvalue"], controls[side .. "CRSRchar"],
          controls[side .. "CRSRchar"])) + 1
        if x > 2 and controls[side .. "CRSRchar"] == 1 then
          x = 2
        elseif x > 2 and controls[side .. "CRSRchar"] == 2 then
          x = 1
        end
        if x > 10 then
          x = 1
    ...
    elseif mode == 2 then
      if controls[side .. "CRSRchar"] == 0 then
        ...
        values[side .. "gstring"] = string.format("%02d#%s#%s", values[side .. "editvalue"]["days"], months,
          values[side .. "editvalue"]["year"])
        values[side .. "bstring"] = values[side .. "gstring"]
  ...
  elseif values[side .. "editstate"] == 2 then
    ...
    if controls["multiselect"] < 1 then
      controls["multiselect"] = values["multipleWPT"]["length"]
    elseif controls["multiselect"] > values["multipleWPT"]["length"] then
      controls["multiselect"] = 1
    end
    if controls["multiselect"] < controls["sview"] then
      controls["sview"] = controls["multiselect"]
    elseif controls["multiselect"] > controls["sview"] + 3 then
      controls["sview"] = controls["multiselect"] - 3
    end


    if controls["sview"] < 1 then
      controls["sview"] = values["multipleWPT"]["length"]
    elseif controls["sview"] > values["multipleWPT"]["length"] then
      controls["sview"] = 1
    end
...
function WPTpage(types, mode, subpage)
  ...
  if controls[CRSR] == 1 then
    ...
    if values[page .. "return"] == 1 and (types == 3 or types == 4) and values["INTref"] ~= "_____" and values["INTrad"] ~= "____" and values["INTdist"] ~= "____" and controls[page .. "select"] < 10 then
      waypoint[1]["lat"], waypoint[1]["lon"] = raddist(values["INTref"]["lat"], values["INTref"]["lon"],
        values["INTrad"] / 10, values["INTdist"] / 10)
    ...
    elseif controls[ent] == 1 and (subpage == 70 or subpage == 71 or subpage == 80) then
      ...
      elseif values["SIDSTARsel"][name .. "RWY"] == nil then
        values["SIDSTARsel"][name .. "RWY"] = values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]
      ...
      if values["SIDSTARsel"]["stat"] == 1 then
        if FPlan[0]["length"] + values["SIDSTARsel"]["num"] > 30 then
          ...
          if subpage == 80 then
            FPlan[0]["APPident"] = appname(values["SIDSTARsel"]["APP"],
              values["SIDSTAR"]["APP"][values["SIDSTARsel"]["APP"]]["RWY"][1], 1)
  ...
  if subpage == 70 or subpage == 71 or subpage == 80 then
    ...
    if subpage ~= 80 or values["SIDSTARsel"]["APPTRANS"] ~= nil then
      if controls[page .. "select"] > controls["SIDSTARview"] + 3 then controls["SIDSTARview"] = controls
        [page .. "select"] - 3 end
    else
      if controls[page .. "select"] > controls["SIDSTARview"] + 4 then controls["SIDSTARview"] = controls
        [page .. "select"] - 4 end
    end
    if controls[page .. "select"] < controls["SIDSTARview"] + 1 then controls["SIDSTARview"] = controls
      [page .. "select"] - 1 end
    if values["SIDSTARsel"][name] == nil then
      ...
      if controls["SIDSTARview"] > values["SIDSTAR"][name]["num"] - 4 then controls["SIDSTARview"] = values["SIDSTAR"]
        [name]["num"] - 4 end
    elseif values["SIDSTARsel"][name .. "RWY"] == nil then
      if controls[page .. "select"] == 0 then
      ...
      if controls["SIDSTARview"] > values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]["num"] - 4 then controls["SIDSTARview"] =
        values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]["num"] - 4 end
    elseif values["SIDSTARsel"][name .. "TRANS"] == nil then
      if controls[page .. "select"] == 0 then
      ...
      if controls["SIDSTARview"] > values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"]["num"] - 4 then controls["SIDSTARview"] =
        values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"]["num"] - 4 end
    else
      if controls[page .. "select"] == 0 then
      ...
      if controls[page .. "select"] < values["SIDSTARsel"]["num"] then
        if controls["SIDSTARview"] > values["SIDSTARsel"]["num"] - 3 then controls["SIDSTARview"] = values["SIDSTARsel"]
          ["num"] - 3 end
      else
        if controls["SIDSTARview"] > values["SIDSTARsel"]["num"] - 4 then controls["SIDSTARview"] = values["SIDSTARsel"]
          ["num"] - 4 end
      end
    end
  ...
  if controls[page .. "select"] == 0 and controls[CRSR] == 1 then
    if controls["SCAN"] == 0 then
      ...
      bline[1] = bline[1] ..
      " " ..
      makelength(string.gsub(string.sub(waypoint[1]["ident"], controls[page .. "CRSRchar"], typeslength), " ", "#"),
        typeslength, 1)
    end
  elseif controls[page .. "select"] == 2 and subpage ~= 70 and subpage ~= 71 and subpage ~= 80 then
    if controls["SCAN"] == 0 then
      ...
      bline[2] = bline[2] ..
      makelength(string.gsub(string.sub(waypoint[1]["name1"], controls[page .. "CRSRchar"], 11), " ", "#"), 11, 1)
    end
  end
                ...
                if RWY:sub(1, 2) == "RW" or RWY == "ALL" then
                  ...
                  if values["SIDSTAR"][name][SIDident] then
                    if TRANSident ~= values["SIDSTAR"][name][SIDident]["TRANS"][values["SIDSTAR"][name][SIDident]["TRANS"]["num"]] then
                      ...
                      values["SIDSTAR"][name][SIDident]["TRANS"]["num"] = values["SIDSTAR"][name][SIDident]["TRANS"]
                      ["num"] + 1
                      values["SIDSTAR"][name][SIDident]["TRANS"][values["SIDSTAR"][name][SIDident]["TRANS"]["num"]] =
                      TRANSident
  ...
  if types == 0 then
    ...
    if subpage == 10 then
      ...
      elseif waypoint["length"] == -1 then
        if waypoint[1]["USR"] == 1 then       --fix to usr airports not shown correctly in nearest list and nil errors
          ...
          gline[5] = string.format("%s     %03d*to", string.sub(gline[5], 0, 12),
            course(values["GPSlat"], values["GPSlon"], waypoint[1]["lat"], waypoint[1]["lon"]))
          gline[6] = string.format("%s     %snm", string.sub(gline[6], 0, 12),
            dynaround(distance(waypoint[1]["lat"], waypoint[1]["lon"], values["GPSlat"], values["GPSlon"]), 4))
        else
          gline[4] = string.format("%s %s' %s", gline[4], makelength(RWYtable[waypoint[1]["ident"]][1]["length"], 5, 1),
            RWYtable[waypoint[1]["ident"]][1]["surf"])
          gline[5] = string.format("%s%s    %03d*to", gline[5], RWYtable[waypoint[1]["ident"]][1]["light"],
            course(values["GPSlat"], values["GPSlon"], waypoint[1]["lat"], waypoint[1]["lon"]))
          gline[6] = string.format("%s     %snm", gline[6],
            dynaround(distance(waypoint[1]["lat"], waypoint[1]["lon"], values["GPSlat"], values["GPSlon"]), 4))
        end
      end
      if mode == 1 then
      ...
      if # APT3Comp == 0 then
        ...
        while RWYnum <= waypoint[1]["RWYs"] do
          if RWYtable[waypoint[1]["ident"]][RWYnum]["lat2"] then
            local dist1 = distance(waypoint[1]["lat"], waypoint[1]["lon"], RWYtable[waypoint[1]["ident"]][RWYnum]["lat"],
              RWYtable[waypoint[1]["ident"]][RWYnum]["lon"])
            if dist1 > PXnm then PXnm = dist1 end
            dist1 = distance(waypoint[1]["lat"], waypoint[1]["lon"], RWYtable[waypoint[1]["ident"]][RWYnum]["lat2"],
              RWYtable[waypoint[1]["ident"]][RWYnum]["lon2"])
            if dist1 > PXnm then PXnm = dist1 end
          end
          RWYnum = RWYnum + 1
          --end
        ...
        while RWYnum <= waypoint[1]["RWYs"] do
          ...
          if RWYtable[waypoint[1]["ident"]][RWYnum]["lat2"] then
            local dist1 = distance(waypoint[1]["lat"], waypoint[1]["lon"], RWYtable[waypoint[1]["ident"]][RWYnum]["lat"],
              RWYtable[waypoint[1]["ident"]][RWYnum]["lon"]) * PXnm
            local CRS1 = course(waypoint[1]["lat"], waypoint[1]["lon"], RWYtable[waypoint[1]["ident"]][RWYnum]["lat"],
              RWYtable[waypoint[1]["ident"]][RWYnum]["lon"])
            local y1 = 61 + cos((CRS1) * pi / 180) * dist1
            local x1 = 156.5 + cos((CRS1 - 90) * pi / 180) * dist1
            dist1 = distance(waypoint[1]["lat"], waypoint[1]["lon"], RWYtable[waypoint[1]["ident"]][RWYnum]["lat2"],
              RWYtable[waypoint[1]["ident"]][RWYnum]["lon2"]) * PXnm
            CRS1 = course(waypoint[1]["lat"], waypoint[1]["lon"], RWYtable[waypoint[1]["ident"]][RWYnum]["lat2"],
              RWYtable[waypoint[1]["ident"]][RWYnum]["lon2"])
    ...
    elseif subpage > 30 and subpage < 40 then
      if waypoint["length"] ~= 0 then
        if waypoint[1]["USR"] == 0 then
          gline[3] = gline[3] ..
          RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2 - 1]["number1"] ..
          "/" ..
          RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2 - 1]["number2"] ..
          "  " .. RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2 - 1]["light"]
          gline[4] = gline[4] ..
          makelength(RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2 - 1]["length"], 6, 1) ..
          "' " .. RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2 - 1]["surf"]
          if waypoint[1]["RWYs"] >= (subpage - 30) * 2 then
            gline[5] = gline[5] ..
            RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2]["number1"] ..
            "/" ..
            RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2]["number2"] ..
            "  " .. RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2]["light"]
            gline[6] = gline[6] ..
            makelength(RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2]["length"], 6, 1) ..
            "' " .. RWYtable[waypoint[1]["ident"]][(subpage - 30) * 2]["surf"]
          end
        else
          ...
          if values["SIDSTARsel"][name] == nil then
            ...
            while line <= 5 do
              if controls["SIDSTARview"] + line - 2 <= values["SIDSTAR"][name]["num"] then
                ...
                makelength(controls["SIDSTARview"] + line - 2, 2, 1) ..
                " " .. makelength(values["SIDSTAR"][name][controls["SIDSTARview"] + line - 2], 8, 0)
            ...
            if controls["SIDSTARview"] + 4 <= values["SIDSTAR"][name]["num"] then
              gline[6] = gline[6] ..
              makelength(values["SIDSTAR"][name]["num"], 2, 1) ..
              " " .. makelength(values["SIDSTAR"][name][values["SIDSTAR"][name]["num"]], 8, 0)
          ...
          elseif values["SIDSTARsel"][name .. "RWY"] == nil then
            ...
            while line <= 5 do
              if controls["SIDSTARview"] + line - 2 <= values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]["num"] then
                ...
                makelength(controls["SIDSTARview"] + line - 2, 2, 1) ..
                " " ..
                makelength(values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"][controls["SIDSTARview"] + line - 2],
                  5, 0)
                if controls["rselect"] == controls["SIDSTARview"] + line - 2 and controls[CRSR] == 1 and values["flash"] == 1 then
                  bline[line] = bline[line] .. string.gsub(string.sub(gline[line], 13, 23), " ", "#")
                end
            ...
            if controls["SIDSTARview"] + 4 <= values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]["num"] then
              gline[6] = gline[6] ..
              makelength(values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]["num"], 2, 1) ..
              " " ..
              makelength(
              values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]
              [values["SIDSTAR"][name][values["SIDSTARsel"][name]]["RWY"]["num"]], 5, 0)
              if controls["rselect"] == controls["SIDSTARview"] + 4 and controls[CRSR] == 1 and values["flash"] == 1 then
                bline[6] = bline[6] .. string.gsub(string.sub(gline[6], 13, 23), " ", "#")
              end
            end
          elseif values["SIDSTARsel"][name .. "TRANS"] == nil then
            ...
            while line <= 5 do
              if controls["SIDSTARview"] + line - 2 <= values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"]["num"] then
                ...
                makelength(controls["SIDSTARview"] + line - 2, 2, 1) ..
                " " ..
                makelength(values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"][controls["SIDSTARview"] + line - 2],
                  5, 0)
                if controls["rselect"] == controls["SIDSTARview"] + line - 2 and controls[CRSR] == 1 and values["flash"] == 1 then
                  bline[line] = bline[line] .. string.gsub(string.sub(gline[line], 13, 23), " ", "#")
                end
            ...
            if controls["SIDSTARview"] + 4 <= values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"]["num"] then
              gline[6] = gline[6] ..
              makelength(values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"]["num"], 2, 1) ..
              " " ..
              makelength(
              values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"]
              [values["SIDSTAR"][name][values["SIDSTARsel"][name]]["TRANS"]["num"]], 5, 0)
              if controls["rselect"] == controls["SIDSTARview"] + 4 and controls[CRSR] == 1 and values["flash"] == 1 then
                bline[6] = bline[6] .. string.gsub(string.sub(gline[6], 13, 23), " ", "#")
              end
            end
          elseif values["SIDSTARsel"]["stat"] == 0 then
            ...
            while line <= 4 do
              if controls["SIDSTARview"] + line - 1 <= values["SIDSTARsel"]["num"] then
                gline[line] = gline[line] ..
                makelength(controls["SIDSTARview"] + line - 1, 2, 1) ..
                " " .. makelength(values["SIDSTARsel"][controls["SIDSTARview"] + line - 1]["ident"], 8, 0)
            ...
            if controls["SIDSTARview"] + 4 <= values["SIDSTARsel"]["num"] then
              gline[5] = gline[5] ..
              makelength(values["SIDSTARsel"]["num"], 2, 1) ..
              " " .. makelength(values["SIDSTARsel"][values["SIDSTARsel"]["num"]]["ident"], 8, 0)
              if controls["rselect"] == controls["SIDSTARview"] + 4 and controls[CRSR] == 1 and values["flash"] == 1 then
      ...
      if values["APTpage"]["length"] ~= 0 then
        if values["SIDSTAR"]["APP"]["num"] == 0 then
          ...
          if values["SIDSTARsel"]["APP"] == nil then
            ...
            while line <= 5 do
              if controls["SIDSTARview"] + line - 1 <= values["SIDSTAR"]["APP"]["num"] then
                ...
                " " ..
                makelength(
                appname(values["SIDSTAR"]["APP"][controls["SIDSTARview"] + line - 1],
                  values["SIDSTAR"]["APP"][values["SIDSTAR"]["APP"][controls["SIDSTARview"] + line - 1]]["RWY"][1], 0), 8,
                  0)
                if controls["rselect"] == controls["SIDSTARview"] + line - 1 and controls[CRSR] == 1 and values["flash"] == 1 then
                  bline[line] = bline[line] .. string.gsub(string.sub(gline[line], -11), " ", "#")
                end
            ...
            if controls["SIDSTARview"] + 5 <= values["SIDSTAR"]["APP"]["num"] then
              gline[6] = gline[6] ..
              makelength(values["SIDSTAR"]["APP"]["num"], 2, 1) ..
              " " ..
              makelength(
              appname(values["SIDSTAR"]["APP"][values["SIDSTAR"]["APP"]["num"]],
                values["SIDSTAR"]["APP"][values["SIDSTAR"]["APP"][values["SIDSTAR"]["APP"]["num"]]]["RWY"][1], 0), 8, 0)
          ...
          elseif values["SIDSTARsel"]["APPTRANS"] == nil then
            gline[1] = string.sub(gline[1], 1, 12) ..
            appname(values["SIDSTARsel"]["APP"], values["SIDSTAR"]["APP"][values["SIDSTARsel"]["APP"]]["RWY"][1], 1) ..
            "-" .. waypoint[1]["ident"]
            ...
            while line <= 5 do
              if controls["SIDSTARview"] + line - 1 <= values["SIDSTAR"]["APP"][values["SIDSTARsel"]["APP"]]["TRANS"]["num"] then
                ...
                " " ..
                makelength(
                values["SIDSTAR"]["APP"][values["SIDSTARsel"]["APP"]]["TRANS"][controls["SIDSTARview"] + line - 1], 5, 0)
                if controls["rselect"] == controls["SIDSTARview"] + line - 1 and controls[CRSR] == 1 and values["flash"] == 1 then
            ...
            if controls["SIDSTARview"] + 5 <= values["SIDSTAR"]["APP"][values["SIDSTARsel"]["APP"]]["TRANS"]["num"] then
              gline[6] = gline[6] ..
              "   " ..
              makelength(values["SIDSTAR"]["APP"][values["SIDSTARsel"]["APP"]]["TRANS"]["num"], 2, 1) ..
              " " ..
              makelength(
              values["SIDSTAR"]["APP"][values["SIDSTARsel"]["APP"]]["TRANS"]
              [values["SIDSTAR"]["APP"][values["SIDSTARsel"]["APP"]]["TRANS"]["num"]], 5, 0)
              if controls["rselect"] == controls["SIDSTARview"] + 5 and controls[CRSR] == 1 and values["flash"] == 1 then
          ...
          elseif values["SIDSTARsel"]["stat"] == 0 then
            ...
            while line <= 4 do
              if controls["SIDSTARview"] + line - 1 <= values["SIDSTARsel"]["num"] then
                gline[line] = gline[line] ..
                makelength(controls["SIDSTARview"] + line - 1, 2, 1) ..
                " " .. makelength(values["SIDSTARsel"][controls["SIDSTARview"] + line - 1]["ident"], 5, 0)
            ...
            if controls["SIDSTARview"] + 4 <= values["SIDSTARsel"]["num"] then
              ...
              makelength(values["SIDSTARsel"]["num"], 2, 1) ..
              " " ..
              makelength(values["SIDSTARsel"][values["SIDSTARsel"]["num"]]["ident"], 5, 0) .. string.char(30) .. "  "
              if controls["rselect"] == controls["SIDSTARview"] + 4 and controls[CRSR] == 1 and values["flash"] == 1 then
                bline[5] = bline[5] .. string.gsub(string.sub(gline[5], 13, 23), " ", "#")
              end
            end
  ...
  elseif types == 1 then
    if values["VORpage"]["length"] ~= 0 then
      if waypoint[1]["USR"] == 0 then
        ...
        gline[4] = gline[4] ..
        string.sub(waypoint[1]["freq"], 1, 3) .. "." .. string.sub(waypoint[1]["freq"], 4, 5) .. magvar
        if values["VORpage"]["length"] > 0 then
          gline[5] = gline[5] .. convertLatLon(waypoint[1]["lat"], 0)
          gline[6] = gline[6] .. convertLatLon(waypoint[1]["lon"], 1)
        else
        ...
        if controls[page .. "select"] == 3 then
          gline[4] = gline[4] ..
          string.sub(values[page .. "gstring"], 1, 3) .. "." .. string.sub(values[page .. "gstring"], 4, 5)
          ...
          bline[4] = bline[4] ..
          string.sub(values[page .. "bstring"], 1, 3) .. "." .. string.sub(values[page .. "bstring"], 4, 5)
        elseif controls[page .. "select"] == 4 then
          gline[4] = gline[4] ..
          string.sub(waypoint[1]["freq"], 1, 3) ..
          "." ..
          string.sub(waypoint[1]["freq"], 4, 5) ..
          " " .. string.sub(values[page .. "gstring"], 1, 2) .. "*" .. string.sub(values[page .. "gstring"], 3, 3)
          bline[4] = bline[4] ..
          "       " .. string.sub(values[page .. "bstring"], 1, 2) .. "*" .. string.sub(values[page .. "bstring"], 3, 3)
        else
  ...
  elseif types == 2 then
    if waypoint["length"] ~= 0 then
      if waypoint[1]["USR"] == 0 then
        ...
        if controls[page .. "select"] == 3 then
          gline[4] = gline[4] ..
          "FREQ " .. string.sub(values[page .. "gstring"], 1, 4) .. "." .. string.sub(values[page .. "gstring"], 5, 5)
          bline[4] = bline[4] ..
          "     " .. string.sub(values[page .. "bstring"], 1, 4) .. "." .. string.sub(values[page .. "bstring"], 5, 5)
        else
          gline[4] = gline[4] ..
          "FREQ " .. string.sub(waypoint[1]["freq"], 1, 4) .. "." .. string.sub(waypoint[1]["freq"], 5, 5)
        end
      end
    end
    if mode == 1 then
    ...
    if waypoint["length"] ~= 0 then
      ...
      if values["INTref"]["ident"] == "_____" and waypoint[1]["lat"] ~= "_" and waypoint[1]["lon"] ~= "_" then
        ...
        if values["CALC3timer"] > 3 then
          ...
          values["INTdist"] = makelength(
          math.floor(distance(values["INTref"]["lat"], values["INTref"]["lon"], waypoint[1]["lat"], waypoint[1]["lon"]) *
          10), 4, 1)
      ...
      elseif values["INTrad"] == "____" and waypoint[1]["lat"] ~= "_" and waypoint[1]["lon"] ~= "_" then
        ...
        if controls[page .. "select"] == 4 then
          gline[3] = gline[3] ..
          "RAD: " ..
          string.sub(values[page .. "gstring"], 1, 3) .. "." .. string.sub(values[page .. "gstring"], 4, 4) .. "*"
          bline[3] = bline[3] ..
          "     " .. string.sub(values[page .. "bstring"], 1, 3) .. "." .. string.sub(values[page .. "bstring"], 4, 4)
        else
          gline[3] = gline[3] ..
          "RAD: " .. string.sub(values["INTrad"], 1, 3) .. "." .. string.sub(values["INTrad"], 4, 4) .. "*"
        end
        if controls[page .. "select"] == 5 then
          gline[4] = gline[4] ..
          "DIS:" ..
          string.sub(values[page .. "gstring"], 1, 3) .. "." .. string.sub(values[page .. "gstring"], 4, 4) .. "nm"
          bline[4] = bline[4] ..
          "    " .. string.sub(values[page .. "bstring"], 1, 3) .. "." .. string.sub(values[page .. "bstring"], 4, 4)
        else
          gline[4] = gline[4] ..
...
function update()
  ...
  elseif rpage == -4 then
    ...
    if controls["rselect"] == 1 then
      if values["HSIinterf"] == 0 then
        gline[3] = string.format("OBS IN ---*|%s:%02d%s", values["rgstring"], values["time"]["second"],
          values["time"]["zonename"])
      else
        gline[3] = string.format("OBS IN %03d*|%s:%02d%s", values["HSIOBS"], values["rgstring"], values["time"]
        ["second"], values["time"]["zonename"])
      end
      ...
      if values["HSIinterf"] == 0 then
        gline[3] = string.format("OBS IN ---*|%02d:%02d:%02d%s", hour2, values["time"]["minute"],
          values["time"]["second"], values["time"]["zonename"])
      else
        gline[3] = string.format("OBS IN %03d*|%02d:%02d:%02d%s", values["HSIOBS"], hour2, values["time"]["minute"],
          values["time"]["second"], values["time"]["zonename"])
      end
    end
    ...
    if values["barounit"] == 0 then
      ...
      gline[5] = string.format("RMI    130*|BARO:%s.%s@", string.sub(values["baro"], 1, 2),
        string.sub(values["baro"], 3, 4))
    else
      if controls["rselect"] == 3 then
  ...
  if lpage == 1 then
    ...
    elseif lsubpage[1] == 10 then
      ...
      if values["TRI1"][2]["ident"] == "     " then
        ...
        local dist = distance(values["TRI1"][1]["lat"], values["TRI1"][1]["lon"], values["TRI1"][2]["lat"],
          values["TRI1"][2]["lon"])
        local crs = course(values["TRI1"][1]["lat"], values["TRI1"][1]["lon"], values["TRI1"][2]["lat"],
          values["TRI1"][2]["lon"])
    ...
    elseif lsubpage[1] == 20 then
      ...
      if values["TRI1"][2]["ident"] == "     " then
        ...
        gline[2] = "ESA " ..
        getMSA(values["TRI1"][1]["lat"], values["TRI1"][1]["lon"], values["TRI1"][2]["lat"], values["TRI1"][2]["lon"]) ..
        "ft|"
      end
      gline[3] = "           |"
      gline[4] = "           |"
    ...
    elseif lsubpage[1] == 30 then
      ...
      if values["TRI3"][1]["ident"] == "     " or values["TRI3"][2]["ident"] == "     " then
        ...
        local dist = distance(values["TRI3"][1]["lat"], values["TRI3"][1]["lon"], values["TRI3"][2]["lat"],
          values["TRI3"][2]["lon"])
        local crs = course(values["TRI3"][1]["lat"], values["TRI3"][1]["lon"], values["TRI3"][2]["lat"],
          values["TRI3"][2]["lon"])
        gline[2] = string.format("%snm %03d*|", makelength(round(dist), 4, 1), crs)
    ...
    elseif lsubpage[1] == 40 then
      ...
      if values["TRI3"][2]["ident"] == "     " then
        ...
        gline[2] = "ESA " ..
        getMSA(values["TRI3"][1]["lat"], values["TRI3"][1]["lon"], values["TRI3"][2]["lat"], values["TRI3"][2]["lon"]) ..
        "ft|"
      end
      gline[3] = "           |"
      gline[4] = "           |"
    ...
    elseif lsubpage[1] == 50 then
      ...
      if FPlan[values["TRI5num"]]["length"] < 2 then
        ...
        gline[2] = FPlan[values["TRI5num"]][1]["ident"] ..
        "-" .. FPlan[values["TRI5num"]][FPlan[values["TRI5num"]]["length"]]["ident"] .. "|"
        if values["TRI5SPD"] == -1 then
          local SWC = (values["TRI0WSpeed"] / values["TRI0TAS"]) * sin((values["TRI0WHead"] - crs) * pi / 180)

          if (math.abs(SWC) > 1) then
          else
            values["TRI5SPD"] = round(values["TRI0TAS"] * math.sqrt(1 - SWC ^ 2) -
            values["TRI0WSpeed"] * cos((values["TRI0WHead"] - crs) * pi / 180))
          end
        end
  ...
  elseif lpage == 3 then
    ...
    if controls["lCRSR"] == 1 then
      ...
      if controls["lselect"] < controls["lview"] + 2 then
        controls["lview"] = controls["lselect"] - 1
    ...
    if controls["lview"] < 0 then
      controls["lview"] = maxlen + 2
    elseif controls["lview"] > maxlen + 2 then
      controls["lview"] = 0
    end
      ...
      if WPTnum == FPlan[lsubpage[3] / 10]["SIDstart"] then
        ...
        if WPTnum == FPlan[lsubpage[3] / 10]["STARstart"] then
          ...
          if WPTnum == FPlan[lsubpage[3] / 10]["APPstart"] then
            if WPTnum == values["activeWPT"]["active"] and controls["lCRSRchar"] == 0 and controls["FPLstate"] == 0 and values["activeWPT"][1]["ident"] ~= "    $" and values["activeWPT"][1]["ident"] ~= "     " then
              gline[line] = '"' ..
              makelength(FPlan[lsubpage[3] / 10]["APPident"] .. "-" .. FPlan[lsubpage[3] / 10]["APPAPT"]["ident"], 10, 0) ..
              "|"
            else
              gline[line] = " " ..
              makelength(FPlan[lsubpage[3] / 10]["APPident"] .. "-" .. FPlan[lsubpage[3] / 10]["APPAPT"]["ident"], 10, 0) ..
              ...
              if WPTnum > 0 then
                if controls["lselect"] == controls["lview"] + line and controls["lCRSR"] == 1 then
                  if controls["FPLstate"] == 1 then
                    ...
                    gline[line] = FplanArrows(line, WPTnum) ..
                    makelength(WPTnum, 2, 1) .. dot .. values["lgstring"] .. suffix(WPTnum, lsubpage[3]) .. " |"
                  ...
                  if WPTnum <= FPlan[lsubpage[3] / 10]["length"] then
                    gline[line] = FplanArrows(line, WPTnum) ..
                    makelength(WPTnum, 2, 1) ..
                    dot .. FPlan[lsubpage[3] / 10][WPTnum]["ident"] .. suffix(WPTnum, lsubpage[3]) .. " |"
    ...
    if controls["lselect"] == controls["lview"] + 6 then
      ...
      if controls["lview"] + 5 <= maxlen and FPlan[lsubpage[3] / 10][FPlan[lsubpage[3] / 10]] ~= nil then                                                                           --fix for error on fpl deletion
        gline[6] = FplanArrows(6) ..
        makelength(FPlan[lsubpage[3] / 10]["length"], 2, 1) ..
        ":" ..
        FPlan[lsubpage[3] / 10][FPlan[lsubpage[3] / 10]["length"]]["ident"] ..
        "  |"                                                                                                                                                                       --errors if active fplan deleted while in obs --no bad efects observed
  ...
  elseif lpage == 4 then
    ...
    elseif lsubpage[4] == 20 then
      ...
      if values["GPSnum"] >= 4 then
        ...
        gline[3] = string.format("%s%03d*fr|", values["REFVOR"]["ident"],
          round(course(values["REFVOR"]["lat"], values["REFVOR"]["lon"], values["GPSlat"], values["GPSlon"])))
        gline[4] = "     " ..
        dynaround(distance(values["REFVOR"]["lat"], values["REFVOR"]["lon"], values["GPSlat"], values["GPSlon"]), 4) ..
        "nm|"
    ...
    elseif lsubpage[4] == 30 then
      ...
      if values["activeWPT"]["length"] < 2 or values["GPSnum"] < 4 then
        ...
        if get(GPSmode) == 1 then
          local wptnum = 3
          local ESA = getMSA(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"],
          ...
          gline[6] = "ESA " ..
          getMSA(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"], values["activeWPT"][2]["lon"]) ..
          "ft|"
        end
      end
    elseif lsubpage[4] == 40 then
      ...
      if values["VNVstat"][0] == 0 then
        ...
        if WPTnum == 1 then
          ...
          values["VNVANG"] = makelength(
          float(
          -math.atan((IndALT - values["VNVSEL"]) * 0.000164578834 /
          (distanceFPLN(values["activeWPT"], 2, WPTnum, 0) - values["VNVOFFS"])) / pi * 180, 1), 4, 1)
        end
  ...
  elseif lpage == 5 then
    ...
    if lsubpage[5] == 10 then
      ...
      if controls["lCRSR"] == 1 then
        ...
        elseif controls["lknobs"] == -1 then
          ...
          elseif controls["lselect"] == 2 then
            if controls["lCRSRchar"] == 1 then
              if string.sub(values["cal1temp"], 1, 1) == "0" then
                values["cal1temp"] = replaceChar(values["cal1temp"], 1, "-")
              else
                values["cal1temp"] = replaceChar(values["cal1temp"], 1, "0")
              end
            else
      ...
      if values["barounit"] == 0 then
        ...
        gline[3] = string.format("BARO:%s.%s'|", string.sub(values["cal1baro"], 1, 2),
          string.sub(values["cal1baro"], 3, 4))
      else
        if controls["lselect"] == 1 then
          if controls["lCRSRchar"] == 1 then
    ...
    elseif lsubpage[5] == 20 then
      ...
      if controls["lCRSR"] == 1 then
        ...
        elseif controls["lknobs"] == -1 then
          ...
          elseif controls["lselect"] == 3 then
            if controls["lCRSRchar"] == 1 then
              if string.sub(values["cal2temp"], 1, 1) == "0" then
                values["cal2temp"] = replaceChar(values["cal2temp"], 1, "-")
              else
                values["cal2temp"] = replaceChar(values["cal2temp"], 1, "0")
              end
            else
      ...
      if values["barounit"] == 0 then
        ...
        gline[4] = string.format("BARO:%s.%s@|", string.sub(values["cal1baro"], 1, 2),
          string.sub(values["cal1baro"], 3, 4))
      else
        if controls["lselect"] == 2 then
          if controls["lCRSRchar"] == 1 then
    ...
    elseif lsubpage[5] == 30 then
      ...
      2)
      local WD = values["GPSTRK"] * pi / 180 +
      math.atan2(values["cal2TAS"] / 1.94384 * sin(get(PSIin) * pi / 180 - values["GPSTRK"] * pi / 180),
        values["cal2TAS"] / 1.94384 * cos(get(PSIin) * pi / 180 - values["GPSTRK"] * pi / 180) - values["GPSSPD"])
      ...
      if controls["lCRSR"] == 1 then
        ...
        elseif controls["lknobs"] == -1 then
          ...
          elseif controls["lselect"] == 2 then
            ...
            values["cal4FPM"] = makelength(round((values["cal4GS"] * 6076 / 60) * math.tan(values["cal4ANG"] * pi / 180),
              -2), 4, 2)
          end
          controls["lknobs"] = 0
        elseif controls["lknobs"] == 1 then
    ...
    elseif lsubpage[5] == 50 then
      if controls["lCRSR"] == 1 then
        ...
        elseif controls["lknobs"] == -1 then
          if controls["lselect"] == 0 then
            if controls["lCRSRchar"] == 1 then
              if string.sub(values["cal5C"], 1, 1) == "0" then
                values["cal5C"] = replaceChar(values["cal5C"], 1, "-")
              else
                values["cal5C"] = replaceChar(values["cal5C"], 1, "0")
              end
            else
          ...
          elseif controls["lselect"] == 1 then
            if controls["lCRSRchar"] == 1 then
              if string.sub(values["cal5F"], 1, 1) == "0" then
                values["cal5F"] = replaceChar(values["cal5F"], 1, "-")
              elseif string.sub(values["cal5F"], 1, 1) == "1" then
                values["cal5F"] = replaceChar(values["cal5F"], 1, "0")
              else
                values["cal5F"] = replaceChar(values["cal5F"], 1, "1")
              end
            else
        ...
        elseif controls["lknobs"] == 1 then
          ...
          elseif controls["lselect"] == 1 then
            if controls["lCRSRchar"] == 1 then
              if string.sub(values["cal5F"], 1, 1) == "0" then
                values["cal5F"] = replaceChar(values["cal5F"], 1, "1")
              elseif string.sub(values["cal5F"], 1, 1) == "-" then
                values["cal5F"] = replaceChar(values["cal5F"], 1, "0")
              else
                values["cal5F"] = replaceChar(values["cal5F"], 1, "-")
              end
    ...
    elseif lsubpage[5] == 70 then
      ...
      values["suntime"]["zonename"], values["suntime"]["zonediff"], values["suntime"]["sunzonenamel"] = timezone(values
      ["sunzone"])
  ...
  elseif lpage == 6 then
    if lsubpage[6] == 10 then
      ...
      while line <= 6 do
        if values["GPSSAT"][line - 2] == nil then
          ...
          gline[line] = string.format("%s%02d%s %02d %02d*|", values["GPSSAT"][line - 2][0], values["GPSSAT"][line - 2]
          [1], values["GPSSAT"][line - 2][2], values["GPSSAT"][line - 2][3], values["GPSSAT"][line - 2][4])
        end
        line = line + 1
      end
    ...
    elseif lsubpage[6] == 11 then
      ...
      while line <= 4 do
        if values["GPSSAT"][line + 4] == nil then
          ...
          gline[line] = string.format("%s%02d%s %02d %02d*|", values["GPSSAT"][line + 4][0], values["GPSSAT"][line + 4]
          [1], values["GPSSAT"][line + 4][2], values["GPSSAT"][line + 4][3], values["GPSSAT"][line + 4][4])
        end
        line = line + 1
      end
    ...
    elseif lsubpage[6] == 40 then
      if controls["lCRSR"] == 0 then
        ...
        if values["PONG"]["rpad"] < 28 then
          values["PONG"]["rpad"] = 28
        elseif values["PONG"]["rpad"] > 74 then
          values["PONG"]["rpad"] = 74
        end
  ...
  elseif lpage == 7 then
    ...
    elseif lsubpage[7] == 20 then
      ...
      if controls["lselect"] == 1 then
        ...
        gline[4] = string.format("%02d:%02d:%02d%s|", hour2, values["time"]["minute"], values["time"]["second"],
          values["time"]["zonename"])
      end
    ...
    elseif lsubpage[7] == 60 then
      ...
      if values["turnanticipation"] < 0 then
        values["turnanticipation"] = 1
      elseif values["turnanticipation"] > 1 then
        values["turnanticipation"] = 0
      end
  ...
  elseif lpage == 8 then
    ...
    elseif lsubpage[8] == 30 then
      ...
      if controls["lCRSR"] == 1 then
        ...
        elseif controls["sENT"] == 1 then
          ...
          local waypoint = enterident(Usertable[controls["lselect"] + 1]["ident"],
            Usertable[controls["lselect"] + 1]["types"], 0, 5, 0, Usertable[controls["lselect"] + 1]["lat"],
            Usertable[controls["lselect"] + 1]["lon"])
        ...
        if controls["lselect"] < 0 then
          controls["lselect"] = Userlength - 1
        elseif controls["lselect"] > Userlength - 1 then
          controls["lselect"] = 0
        end
        if controls["lselect"] < controls["lview"] then
          controls["lview"] = controls["lselect"]
        elseif controls["lselect"] > controls["lview"] + 4 then
      ...
      while line <= 6 do
        if controls["lview"] - 1 + line <= Userlength then
          if FPLNused[controls["lview"] - 1 + line] == -1 then
            gline[line] = string.format("%s %s    |", Usertable[controls["lview"] - 1 + line]["ident"],
              string.sub(type2typename(Usertable[controls["lview"] - 1 + line]["types"]), 1, 1))
          else
            gline[line] = string.format("%s %s  %s|", Usertable[controls["lview"] - 1 + line]["ident"],
              string.sub(type2typename(Usertable[controls["lview"] - 1 + line]["types"]), 1, 1),
              makelength(FPLNused[controls["lview"] - 1 + line], 2, 1))
          end
        else
          gline[line] = "           |"
        end
    ...
    elseif lsubpage[8] == 40 then
      ...
      if controls["lCRSR"] == 1 then
        ...
        if controls["lselect"] < 0 then
          controls["lselect"] = RMKlength - 1
        elseif controls["lselect"] > RMKlength - 1 then
          controls["lselect"] = 0
        end
        if controls["lselect"] < controls["lview"] then
          controls["lview"] = controls["lselect"]
    ...
    elseif lsubpage[8] == 70 then
      ...
      elseif get(EnginesNum) == 2 then
        ...
        gline[6] = string.format("TOTAL%s|",
          makelength(makelength(round((get(FuelFlow1) + get(FuelFlow2)) * 3600 * multi), 5, 0), 6, 1))
      elseif get(EnginesNum) == 3 then
        ...
        gline[6] = string.format("TOTAL%s|",
          makelength(makelength(round((get(FuelFlow1) + get(FuelFlow2) + get(FuelFlow3)) * 3600 * multi), 5, 0), 6, 1))
      else
        gline[2] = "           |"
        gline[4] = "           |"
        gline[5] = "           |"
        gline[6] = string.format("     %s|",
          makelength(
          makelength(
          round((get(FuelFlow1) + get(FuelFlow2) + get(FuelFlow3) + get(FuelFlow4) + get(FuelFlow5) + get(FuelFlow6) + get(FuelFlow7) + get(FuelFlow8)) *
          3600 * multi), 5, 0), 6, 1))
      end
      gline[7] = "OTH 7"
    ...
    elseif lsubpage[8] == 80 then
      ...
      elseif get(EnginesNum) == 3 then
        ...
        gline[6] = string.format("TOTAL%s|",
          makelength(round((values["fuelused1"] + values["fuelused2"] + values["fuelused3"]) * multi), 6, 1))
      else
        gline[2] = "           |"
        gline[4] = "           |"
        gline[5] = "           |"
        gline[6] = string.format("     %s|",
          makelength(
          round((values["fuelused1"] + values["fuelused2"] + values["fuelused3"] + values["fuelused4"] + values["fuelused5"] + values["fuelused6"] + values["fuelused7"] + values["fuelused8"]) *
          multi), 6, 1))
      end
      gline[7] = "OTH 8"
    ...
    elseif lsubpage[8] == 90 then
      ...
      local WS = math.sqrt((get(TASin) - values["GPSSPD"]) ^ 2 +
      4 * (get(TASin) * values["GPSSPD"]) * (sin((get(PSIin) * pi / 180 - values["GPSTRK"] * pi / 180) / 2)) ^ 2)
      local WD = values["GPSTRK"] * pi / 180 +
      math.atan2(get(TASin) * sin(get(PSIin) * pi / 180 - values["GPSTRK"] * pi / 180),
  ...
  if not (lpage == 7 and lsubpage[7] < 10) and not (lpage == 6 and lsubpage[6] == 40 and controls["lCRSR"] == 1) then
        ...
        if controls["rCRSR"] == 1 and FPlan[lsubpage[3] / 10]["length"] > 1 then
          ...
          if values["reditstate"] == 3 then
            ...
            local dist1 = distance(FPlan[lsubpage[3] / 10][near - 1]["lat"], FPlan[lsubpage[3] / 10][near - 1]["lon"],
              FPlan[lsubpage[3] / 10][near]["lat"], FPlan[lsubpage[3] / 10][near]["lon"])
            ...
            if dist1 < dist2 then
              ...
              values["INTdist"] = makelength(
              math.floor(distance(values["INTref"]["lat"], values["INTref"]["lon"], WPT[1]["lat"], WPT[1]["lon"]) * 10),
                4, 1)
              values["INTrad"] = string.format("%04d",
      ...
      if rsubpage[4] == 10 then
        ...
        if lpage == 3 then
                      ...
                      if values["activeWPT"]["active"] > 0 then
                        if WPTnum <= FPlan[0]["length"] and WPTnum >= values["activeWPT"]["active"] then
                          if FPlan[0][WPTnum]["ident"] ~= "     " then
                            ...
                            gline[line] = string.sub(gline[line], 1, 12) ..
                    ...
                    elseif stop == 0 then
                      if WPTnum <= FPlan[lsubpage[3] / 10]["length"] then
                        if FPlan[lsubpage[3] / 10][WPTnum]["ident"] ~= "     " then
                          gline[line] = string.sub(gline[line], 1, 12) ..
                          makelength(round(distanceFPLN(FPlan[lsubpage[3] / 10], 1, WPTnum, 1)), 3, 1)
                        end
                      else
                        stop = 1
                      end
          ...
          if values["activeWPT"] ~= nil and values["activeWPT"][1] ~= nil and values["activeWPT"]["active"] >= 2 and values["activeWPT"][1]["ident"] ~= "" then
            if get(WPTalert) == 1 and values["flash"] == 0 then
              gline[1] = gline[1] ..
              "  " .. makelength(values["activeWPT"]["active"], 2, 1) .. " " .. values["activeWPT"][2]["ident"]
            else
              gline[1] = gline[1] ..
              " =" .. makelength(values["activeWPT"]["active"], 2, 1) .. " " .. values["activeWPT"][2]["ident"]
            end

            local dist = distance(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"],
              values["activeWPT"][2]["lon"])
            ...
            if dest > 2 then
              gline[4] = gline[4] ..
              "  " .. makelength(dest + values["activeWPT"]["active"] - 2, 2, 1) ..
              " " .. values["activeWPT"][dest]["ident"]
      ...
      elseif rsubpage[4] == 20 then
        if lpage == 3 then
          ...
          if values["activeWPT"] ~= nil and values["activeWPT"][1] ~= nil and values["activeWPT"]["active"] >= 2 and values["activeWPT"][1]["ident"] ~= "" then
            ...
            gline[2] = gline[2] .. "DIS  " .. makelength(makelength(round(dist), 3, 1), 4, 0) .. "NM"
            gline[3] = gline[3] ..
            "   " .. FplnETA(dist / (values["GPSSPD"] * 1.94384449) * 3600) .. values["time"]["zonename"]
            local dest = values["activeWPT"]["length"]
            if FPlan[0]["APPstart"] < 50 and values["activeWPT"]["active"] <= FPlan[0]["APPMAP"] then
              dest = FPlan[0]["APPMAP"] - values["activeWPT"]["active"] + 1
            end
        ...
        if lpage == 3 then
                  ...
                  if WPTnum ~= FPlan[lsubpage[3] / 10]["APPMAP"] then
                    ...
                    if lsubpage[3] == 0 and stop == 0 then
                      if values["activeWPT"]["active"] > 0 then
                        if WPTnum <= FPlan[0]["length"] and WPTnum >= values["activeWPT"]["active"] then
                          if FPlan[0][WPTnum]["ident"] ~= "     " then
                            gline[line] = string.format("%s%s   %03d*", string.sub(gline[line], 1, 12),
                              makelength(
                              makelength(round(distanceFPLN(FPlan[0], values["activeWPT"]["active"], WPTnum, 0)), 3, 1),
                                4, 0), round(courseFPLN(FPlan[0], WPTnum)))
                          else
                            stop = 1
                          end
                        end
                    ...
                    elseif stop == 0 then
                      if WPTnum <= FPlan[lsubpage[3] / 10]["length"] then
                        if FPlan[lsubpage[3] / 10][WPTnum]["ident"] ~= "     " then
                          gline[line] = string.format("%s%s   %03d*", string.sub(gline[line], 1, 12),
                            makelength(makelength(round(distanceFPLN(FPlan[lsubpage[3] / 10], 1, WPTnum, 1)), 3, 1), 4, 0),
                            round(courseFPLN(FPlan[lsubpage[3] / 10], WPTnum)))
                        end
                      else
                        stop = 1
                      end
          ...
          if values["activeWPT"] ~= nil and values["activeWPT"][1] ~= nil and values["activeWPT"]["active"] >= 2 and values["activeWPT"][1]["ident"] ~= "" then
            ...
            if values["activeWPT"]["length"] >= 3 then
              gline[4] = gline[4] ..
              "  " .. makelength(values["activeWPT"]["active"] + 1, 2, 1) .. " " .. values["activeWPT"][3]["ident"]
    ...
    elseif rpage == 5 then
      ...
      if rsubpage[5] == 10 then
        ...
        if lpage == 4 and lsubpage[4] == 10 and controls["DCT"] == 0 then
          ...
          if values["activeWPT"]["length"] < 2 or values["GPSnum"] < 4 then
            ...
            if get(WPTalert) == 1 and values["flash"] == 0 then
              gline[1] = "      " ..
              values["activeWPT"][1]["ident"] ..
              " " .. values["activeWPT"][2]["ident"] .. suffix(values["activeWPT"]["active"], 0)
            else
              gline[1] = "      " ..
              values["activeWPT"][1]["ident"] ..
              "=" .. values["activeWPT"][2]["ident"] .. suffix(values["activeWPT"]["active"], 0)
      ...
      elseif rsubpage[5] == 30 then
        gline[5] = gline[5] ..
        "MSA " .. getMSA(values["GPSlat"], values["GPSlon"], values["GPSlat"], values["GPSlon"]) .. "ft"
        ...
        if lpage == 4 and lsubpage[4] == 50 and controls["lspage"] == 0 and controls["DCT"] == 0 then
          ...
          if values["NAV5RNG"] == 0 then
            ...
            if values["activeWPT"]["length"] == 2 or values["activeWPT"]["active"] == FPlan[0]["APPMAP"] - 1 then
              dist1 = distance(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"],
                values["activeWPT"][2]["lon"]) * 1.05
            elseif values["activeWPT"]["length"] > 2 then
              dist1 = distance(values["GPSlat"], values["GPSlon"], values["activeWPT"][3]["lat"],
                values["activeWPT"][3]["lon"]) * 1.05
            end
            ...
            if string.sub(values["NAV5LNS"], 1, 1) == "1" then
              ...
              if values["activeWPT"]["length"] < 2 or values["GPSnum"] < 4 then
                ...
                if string.sub(ETE, 1, 1) ~= " " then
                  ...
                  gline[5] = "èé" ..
                  string.sub(convtime(values["dist"] / (values["GPSSPD"] * 1.94384449) * 3600), 2, 5) .. "|"
                end
              end
            elseif string.sub(values["NAV5LNS"], 1, 1) == "2" then
              --XTK
          ...
          elseif string.sub(values["NAV5LNS"], 2, 2) == "2" then
            ...
            if values["activeWPT"]["length"] < 2 or values["GPSnum"] < 4 then
              ...
              gline[6] = string.format("óô%03d*|",
                round(course(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"],
                  values["activeWPT"][2]["lon"])))
            end
            --RAD
          elseif string.sub(values["NAV5LNS"], 2, 2) == "3" then
            if values["activeWPT"]["length"] < 2 or values["GPSnum"] < 4 then
              ...
              gline[6] = string.format("õö%03d*|",
                round(course(values["activeWPT"][2]["lat"], values["activeWPT"][2]["lon"], values["GPSlat"],
                  values["GPSlon"])))
            end
          end
          ...
          if controls["rCRSR"] == 1 then
            ...
            elseif controls["rknobs"] == -1 then
              ...
              if controls["rCRSRchar"] == 1 and x < 1 then
                x = 4
              elseif controls["rCRSRchar"] == 2 and x < 1 then
                x = 2
              elseif controls["rCRSRchar"] == 3 and x < 1 then
                x = 2
              elseif controls["rCRSRchar"] == 4 and x < 1 then
                x = 4
              end
            ...
            elseif controls["rknobs"] == 1 then
              x = string2value(string.sub(values["NAV5SHOW"], controls["rCRSRchar"], controls["rCRSRchar"])) + 1
              if controls["rCRSRchar"] == 1 and x > 4 then
                x = 1
              elseif controls["rCRSRchar"] == 2 and x > 2 then
                x = 1
              elseif controls["rCRSRchar"] == 3 and x > 2 then
                x = 1
              elseif controls["rCRSRchar"] == 4 and x > 4 then
                x = 1
              end
              values["NAV5SHOW"] = replaceChar(values["NAV5SHOW"], controls["rCRSRchar"], value2string(x))
              controls["rknobs"] = 0
          ...
          if values["NAV5ORIS"] == 0 then
            ...
            gline[6] = string.format("%s%03d*   %s", gline[6], values["NAV5ORI2"],
              makelength(num2range(values["NAV5RNG2"]), 4, 1))
          end
    ...
    if values["lsreturn"] == 1 then
      ...
      if get(GPSmode) == 1 then
        ...
        if controls["DCT"] == 1 then
          if values["HSIinterf"] ~= 1 then
            values["HSIOBS"] = course(values["GPSlat"], values["GPSlon"], values["direct"]["lat"],
              values["direct"]["lon"])
            set(HSIOBS, values["HSIOBS"])
            if values["NAVSYNC"] == 1 then
              set(NAVOBS, values["HSIOBS"])
            ...
            values["statusmessage"] = string.format("$=#CRS#%03d#",
              round(course(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"],
                values["activeWPT"][2]["lon"])))
            values["statustimer"] = 5
          end
  ...
  if controls["lspage"] == 2 then
    ...
    if controls["sview"] <= values["multipleWPT"]["length"] then
      gline[3] = string.format("%s %s %s?|", makelength(controls["sview"], 2, 1),
        ...
        ICAOtocountry(values["multipleWPT"][controls["sview"]]["country"])) .. string.sub(gline[3], 13)
    else
      gline[3] = "           |" .. string.sub(gline[3], 13)
    end
    if controls["sview"] + 1 <= values["multipleWPT"]["length"] then
      gline[4] = string.format("%s %s %s?|", makelength(controls["sview"] + 1, 2, 1),
        type2typename(values["multipleWPT"][controls["sview"] + 1]["types"]),
        ICAOtocountry(values["multipleWPT"][controls["sview"] + 1]["country"])) .. string.sub(gline[4], 13)
    else
      gline[4] = "           |" .. string.sub(gline[4], 13)
    end

    if controls["sview"] + 2 <= values["multipleWPT"]["length"] then
      gline[5] = string.format("%s %s %s?|", makelength(controls["sview"] + 2, 2, 1),
        type2typename(values["multipleWPT"][controls["sview"] + 2]["types"]),
        ICAOtocountry(values["multipleWPT"][controls["sview"] + 2]["country"])) .. string.sub(gline[5], 13)
    else
      gline[5] = "           |" .. string.sub(gline[5], 13)
    end
    if controls["sview"] + 3 <= values["multipleWPT"]["length"] then
      gline[6] = string.format("%s %s %s?|", makelength(controls["sview"] + 3, 2, 1),
        type2typename(values["multipleWPT"][controls["sview"] + 3]["types"]),
        ICAOtocountry(values["multipleWPT"][controls["sview"] + 3]["country"])) .. string.sub(gline[6], 13)
    else
      gline[6] = "           |" .. string.sub(gline[6], 13)
    end
    gline[7] = "     " .. string.sub(gline[7], 6)
  ...
  if get(power) == 1 then
    ...
    if values["GPSnum"] < 8 then
      if values["date"]["year"] ~= tonumber(os.date("%y")) then
        ...
        local time1 = (((monthstodays(values["date"]["month"], values["date"]["days"]) * 24 + values["time"]["hour"]) * 60) + values["time"]["minute"]) *
        60 + values["time"]["second"]
        local time2 = (((monthstodays(get(monthin), get(dayin)) * 24 + get(hourin)) * 60) + get(minutein)) * 60 +
        get(secondin)
      ...
      if values["GPStime"] > values["realGPS"] * (values["GPSnum"] + 1) then
        ...
        while GPSnum <= (values["GPSnum"]) do
          if values["GPSSAT"][values["GPSnum"]] == nil then
            ...
            values["GPSSAT"][values["GPSnum"]][4] = -(10 / 12960000) * values["GPSSAT"][values["GPSnum"]][5] ^ 2 +
            values["GPSSAT"][values["GPSnum"]][6]
            values["GPSSAT"][values["GPSnum"]][3] = -(10 / (2332800 * (100000 / (values["GPSSAT"][values["GPSnum"]][7] ^ 2)))) *
            values["GPSSAT"][values["GPSnum"]][5] ^ 2 + values["GPSSAT"][values["GPSnum"]][7]
    ...
    while GPSnum <= 8 do
      if values["GPSSAT"][GPSnum] ~= nil then
        if values["GPSSAT"][GPSnum][4] < 5 then
          ...
          values["GPSSAT"][GPSnum][3] = -(10 / (2332800 * (100000 / (values["GPSSAT"][GPSnum][7] ^ 2)))) *
          values["GPSSAT"][GPSnum][5] ^ 2 + values["GPSSAT"][GPSnum][7]
        end
      end
      GPSnum = GPSnum + 1
    end
    ...
    if values["CALCtimer"] >= 1 / values["GPSrate"] then
      ...
      if values["GPSnum"] >= 4 then
        ...
        if values["activeWPT"]["length"] >= 2 then
          ...
          bearing = course(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"],
            values["activeWPT"][2]["lon"])
          ...
          if get(GPSmode) == 1 then
            ...
            local f1 = -values["dist"] /
            distance(values["activeWPT"][1]["lat"], values["activeWPT"][1]["lon"], values["activeWPT"][2]["lat"],
              values["activeWPT"][2]["lon"]) + 1
              ...
              if values["dist"] > values["WPTalertdist"] then
                ...
                if values["activeWPT"]["length"] > 2 and values["activeWPT"]["active"] ~= FPlan[0]["APPMAP"] - 1 then
                  ...
                  if values["activeWPT"]["length"] == 1 then
                    ...
                    values["dist"] = distance(values["GPSlat"], values["GPSlon"], values["activeWPT"][2]["lat"],
                      values["activeWPT"][2]["lon"])
                    local CRS = course(values["activeWPT"][1]["lat"], values["activeWPT"][1]["lon"],
                      values["activeWPT"][2]["lat"], values["activeWPT"][2]["lon"])
                    local CRS2 = CRS - values["DTK"]
                    if CRS2 < -180 then
                      CRS2 = CRS2 + 360
                    elseif CRS2 > 180 then
                      CRS2 = CRS2 - 360
                    end
...
function draw()
  if draw3d == true then
    ...
    if KLN90B:isVisible() or KLN90B_DISPLAY:isVisible() then
      ------------------------------------------------------------------------------------
      sasl.gl.getTargetTextureData(display, currentPosition[1], currentPosition[2], currentPosition[3],
        currentPosition[4])
    end
  else
